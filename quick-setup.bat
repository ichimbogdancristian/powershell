...
REM Download repository using PowerShell
powershell -Command "try { Invoke-WebRequest -Uri '%REPO_URL%/archive/refs/heads/main.zip' -OutFile '%ZIP_FILE%' -UseBasicParsing; Write-Host '[OK] Repository downloaded successfully' -ForegroundColor Green } catch { Write-Host '[ERROR] Download failed:' $_.Exception.Message -ForegroundColor Red; exit 1 }"
if %errorlevel% neq 0 (
...
cd /d "%EXTRACT_DIR%"
powershell -ExecutionPolicy Bypass -Command "& '.\quick-install.ps1'; if ($LASTEXITCODE -ne 0) { exit 1 }"
...
powershell -Command "try { Get-Module posh-git,Terminal-Icons -ListAvailable -ErrorAction Stop | Out-Null; Write-Host '[OK] Essential modules available' -ForegroundColor Green } catch { Write-Host '[WARNING] Some modules may not be installed properly' -ForegroundColor Yellow }"

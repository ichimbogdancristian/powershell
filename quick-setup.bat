...
powershell -NoProfile -Command "$ProgressPreference='SilentlyContinue'; try { Expand-Archive '%TEMP_DIR%\repo.zip' '%TEMP_DIR%' -Force; Write-Host '[OK] Extracted' -ForegroundColor Green } catch { Write-Host '[ERROR] Extract failed' -ForegroundColor Red; exit 1 }"
